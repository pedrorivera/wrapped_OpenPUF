...
set vReadVerilogOpts ""
if {[info exist ::env(READ_VERILOG_OPTS)]} {
	foreach opt $::env(READ_VERILOG_OPTS) {
		lappend vReadVerilogOpts "-$opt"
	}
	set vReadVerilogOpts [join $vReadVerilogOpts]
}
...
for { set i 0 } { $i < [llength $::env(VERILOG_FILES)] } { incr i } {
	read_verilog -sv {*}$vReadVerilogOpts {*}$vIdirsArgs [lindex $::env(VERILOG_FILES) $i]
}
